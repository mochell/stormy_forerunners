...
class pwelch(object):
    def __init__(self,data,dt,L=None, ov=None,window=None, save_chunks=False, plot_chunks=False, periodogram=False, prewhite=None):
        ...
        calc_freq(self)
        #print(data.size, L, ov, int(L-ov) )
        nbin=int(np.floor(data.size/(L-ov)))
        #print(nbin)
        if periodogram:
            self.nbin=nbin
        ...
        k=0
        #print('iter range', np.arange(0,data.size,int(L-ov)))
        for i in np.arange(0,data.size-int(L-ov)+1,int(L-ov)):

            if (plot_chunks) and (i >= data.size-6*int(L-ov)):
                M.figure_axis_xy()
